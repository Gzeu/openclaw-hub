...
export default function RootLayout({
  ...
  return (
    <html lang="en" className={`dark ${inter.variable}`}>
      {/*
        Apply bg directly on body via style prop — this is NOT purgeable by Tailwind
        since it’s an inline style, not a class. Guarantees dark bg even if CSS vars
        take a moment to resolve on first paint.
      */}
      <body
          ...
          <div className="max-w-[1440px] mx-auto px-4 sm:px-6 h-14 flex items-center gap-2">
            ...
            <nav className="flex items-center gap-0.5 flex-1 overflow-x-auto no-scrollbar">
              {NAV_LINKS.map(({ href, label, icon }) => (
                <Link
                  key={href}
                  href={href}
                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all duration-150 hover:bg-[var(--bg-hover)] hover:text-white"
            ...
            <div className="flex items-center gap-2 ml-2 shrink-0">
              <a
                ...
                className="badge badge-green text-xs hidden md:inline-flex"
              >
