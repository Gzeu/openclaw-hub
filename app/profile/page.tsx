    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-zinc-400 text-sm animate-pulse font-medium">⏳ Se încarcă profilul profesional...</div>
        </div>
      </div>
    );
  }
  ...
  if (!profile?.mxAddress) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-zinc-900/50 border border-zinc-800 rounded-3xl p-10 text-center space-y-8 backdrop-blur-xl shadow-2xl">
          <div className="mx-auto w-20 h-20 bg-zinc-800/50 rounded-2xl flex items-center justify-center text-4xl shadow-inner">🔒</div>
          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-white tracking-tight">Sesiune expirată</h2>
            <p className="text-zinc-400">Autentifică-te pentru a accesa centrul de comandă OpenClaw.</p>
          </div>
          <Link 
            href="/login" 
            className="w-full inline-flex items-center justify-center gap-2 px-6 py-4 bg-white text-black font-bold rounded-2xl hover:bg-zinc-200 transition-all active:scale-95 shadow-lg shadow-white/5"
          >
            Conectează-te acum
          </Link>
        </div>
    ...
    <div className="max-w-6xl mx-auto px-6 py-12 lg:py-16 space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-zinc-800/50 pb-10">
        <div className="flex items-start gap-6">
          <div className="w-24 h-24 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 rounded-3xl border border-cyan-500/30 flex items-center justify-center text-4xl shadow-lg shadow-cyan-500/5">
        ...
        <div className="flex gap-3">
          ...
          <button 
            ...
            className="flex-1 md:flex-none inline-flex items-center justify-center gap-2 px-5 py-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm font-semibold hover:bg-red-500/20 transition-all"
          >
              ...
              <div className="relative group/addr">
                ...
                <div className="flex items-center gap-4 pt-3 pl-1">
                  ...
                  <a href={`https://explorer.multiversx.com/accounts/${profile.mxAddress}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] text-zinc-400 hover:text-white uppercase tracking-wider font-bold transition-colors">
                    Vezi în Explorer <ExternalLink size={10} />
                  </a>
                </div>
              </div>
            </div>
            ...
            {profile.apiKey ? (
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={copyApiKey}
                    className={`w-14 shrink-0 flex items-center justify-center rounded-2xl border transition-all ${
                      copied 
                ...
                <div className="p-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl">
                  <p className="text-zinc-500 text-[11px] leading-relaxed">
                    <strong className="text-zinc-300">Notă Security:</strong> Folosește acest header pentru apelurile API automate către Hub. 
                    Nu partaja această cheie cu nimeni. Exemplu: <code className="text-cyan-400 px-1 rounded bg-black/20">x-api-key: [key]</code>
                  </p>
                </div>
              </div>
            ) : (
        ...
        <div className="space-y-8">
          ...
          <section className="bg-gradient-to-br from-cyan-600 to-blue-700 rounded-3xl p-8 text-white shadow-xl shadow-cyan-950/20 relative overflow-hidden">
            ...
            <div className="relative space-y-6">
              ...
              <Link 
                href="/economy" 
                className="w-full inline-flex items-center justify-center gap-2 py-3 bg-black/20 hover:bg-black/30 border border-white/10 rounded-xl text-sm font-bold transition-all backdrop-blur-sm"
              >
          ...
          {/* Quick Nav */}
          <section className="bg-zinc-900/40 border border-zinc-800/60 rounded-3xl p-8 space-y-6">
            <h2 className="text-xs font-bold uppercase tracking-widest font-mono text-zinc-400">Scurtături Dashboard</h2>
            <nav className="grid grid-cols-1 gap-2">
              <Link href="/plugins" className="flex items-center gap-3 p-3 rounded-xl bg-zinc-800/30 border border-zinc-800/50 text-zinc-300 hover:text-white hover:bg-zinc-800/60 transition-all text-sm font-medium">
                <Puzzle size={18} className="text-purple-400" /> Catalog Plugin-uri
              </Link>
              <Link href="/wallet" className="flex items-center gap-3 p-3 rounded-xl bg-zinc-800/30 border border-zinc-800/50 text-zinc-300 hover:text-white hover:bg-zinc-800/60 transition-all text-sm font-medium">
                <Coins size={18} className="text-cyan-400" /> Portofel Digital
              </Link>
              <Link href="/activity" className="flex items-center gap-3 p-3 rounded-xl bg-zinc-800/30 border border-zinc-800/50 text-zinc-300 hover:text-white hover:bg-zinc-800/60 transition-all text-sm font-medium">
                <Activity size={18} className="text-green-400" /> Activitate Rețea
              </Link>
            </nav>
          </section>
        </div>
